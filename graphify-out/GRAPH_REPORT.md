...
## Corpus Check
- 119 files · ~68,446 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1589 nodes · 4067 edges · 99 communities (79 shown, 20 thin omitted)
- Extraction: 91% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 351 edges (avg confidence: 0.52)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f58fbc8a`
...
- `_PendingHandshake` --uses--> `ErrorMsg`  [INFERRED]
  hdp-bridge/hdp_bridge/connection.py → hdp-spec/hdp_proto/messages.py
...
### Community 0 - ".from_wire"
Cohesion: 0.11
Nodes (27): _capture_explicit_credential_handler(), Capture the hello so the explicit, non-persistent credential contract is…, EnvelopeError, Any, ValueError, The HDP/0 envelope: `{"hdp", "type", "id", "ts", "corr", "payload"}` — see…, Return a plain `dict`, never a string — serialization belongs to the transport…, Raised by `Envelope.from_wire` on any structurally or semantically invalid… (+19 more)
...
Cohesion: 0.08
Nodes (46): Process, bridge(), bridge_log(), _bridge_proc(), bridge_url(), _hermes_home(), fixture, Shared fixtures for the M2 conformance suite (m1-plan.md §7, m2-plan.md). HDP… (+38 more)

### Community 3 - ".new"
Cohesion: 0.15
Nodes (27): read_frame(), write_frame(), control_request(), Send `ctl_devices_revoke` and return the daemon's reply envelope, or `None` if…, Make one operator control-plane request without importing the plugin transport., _revoke_via_control_socket(), _ctl_invoke_envelope(), Finding I3, server half: `_handle` used to `await self._dispatch(...)` inline… (+19 more)

### Community 4 - "Registry"
Cohesion: 0.11
Nodes (34): Per-connection lifecycle for one node's WebSocket. One `NodeConnection` per…, SQLite-backed device registry (§3, §3.1). `online` state is never persisted —…, Return active device ids valid for policy references., No pairing exists yet (M2 pairing work) — a device enters this table only by…, Insert-or-replace the device row and fully replace its capability set (FR-8's…, Registry, CapabilityRecord, DeviceRecord (+26 more)
...
### Community 9 - "ids.py"
Cohesion: 0.16
Nodes (19): _decode_crockford(), _encode_crockford(), InvalidULIDError, is_valid(), new(), parse(), ValueError, Hand-rolled ULID mint/parse — stdlib only, no `ulid` dependency (SDR-3). A ULID… (+11 more)

### Community 10 - "control.py"
Cohesion: 0.11
Nodes (28): ApprovalScope, ApprovalState, StrEnum, Terminal approval outcomes persisted to the registry., The permitted scope choices for an approval decision., Raised when an approval is no longer pending., UnknownApprovalError, _InvokeReq (+20 more)

### Community 11 - "test_cli.py"
Cohesion: 0.11
Nodes (30): _control_request(), main(), Path, Thin renderer over `operations.revoke` — the daemon-reachable/offline-fallback…, _run_approval_resolve(), _run_approvals_list(), _run_audit_tail(), _run_devices_revoke() (+22 more)

### Community 12 - "InvokeRequest"
Cohesion: 0.09
Nodes (24): The plugin-side entry to daemon-owned invocation-time resolution., DeviceInfo, InvokeRequest, PendingApproval, `BridgeTransport` — the ADR-0004 extraction seam. `engine.py` depends on this…, One unresolved capability invocation for daemon-side live resolution. Carries…, An HDP `pending_approval` state (seed §17). Unreachable at M0 — the stub never…, InprocTransport (+16 more)

### Community 13 - "InvocationsMem"
Cohesion: 0.06
Nodes (34): Connection, WebSocketResponse, InvocationsMem, Any, The bridge-side pending-invocation table — id → in-flight state. Backs real…, Called when a `result` frame arrives. Returns `False` for an unknown id — the…, Fail every pending invocation regardless of device — used on transport shutdown…, A device's connection dropped (or was revoked): fail every invocation still… (+26 more)
...
Cohesion: 0.12
Nodes (18): AuditWriter, Path, Append-only JSONL audit writer (§3.5, §6.3). `O_APPEND|O_CREAT`, 0600, one JSON…, Today's audit records, parsed. Backs `control.py`'s `ctl_audit_tail` verb —…, _error_detail(), Operator-surface orchestration, owned by `hdp_bridge` and shared by both…, Revoke `device_id`, returning the line the caller should render. Prefers the…, revoke() (+10 more)
...
Cohesion: 0.09
Nodes (27): pair_new(), Mint a pairing code and return it in plaintext. There is no control-plane verb…, _apply_pragmas(), connect(), _migrate(), Connection, Path, RuntimeError (+19 more)
...
Cohesion: 0.19
Nodes (16): `hermes hdp {status,devices,pair,audit}` — the CLI-native renderer of `hdp-…, Calls `ctl_audit_tail` over the control socket rather than reading…, CLI-only (finding I1) — `/hdp pair --new` no longer reaches this; see…, render_approvals(), render_audit(), render_pair_new(), render_policy(), render_status() (+8 more)
...
### Community 31 - "messages.py"
Cohesion: 0.09
Nodes (19): Ack, CancelMsg, ErrorMsg, Heartbeat, ProgressMsg, Any, Typed payload dataclasses for each HDP/0 message type (hdp-spec/HDP-0.md §2).…, Either direction's `ack` frame. Empty payload — the envelope's `corr` carries… (+11 more)
...
Cohesion: 0.15
Nodes (12): test_hello_with_a_returning_credential_resolves_the_same_device_id(), _revoking_handler(), Bridge → Node, reply to a successful `hello`. `credential` (M2, §4.3) is…, Welcome, test_welcome_credential_defaults_to_none_and_round_trips_present_as_null(), AndroidNodeFixture, Connect and complete pairing or credential authentication., Reconnect with the stored credential and preserve the bridge-issued identity. (+4 more)

### Community 38 - "mint_pairing_code"
Cohesion: 0.12
Nodes (37): burn_attempt(), code_is_live(), consume_pairing_code(), _hash_code(), mint_pairing_code(), Connection, _random_code(), Pairing-code minting, the online-guessing budget, and atomic consumption… (+29 more)
...
Cohesion: 0.13
Nodes (21): Event, AlreadyRunningError, _check_and_claim_pid(), main(), _pid_is_live(), RuntimeError, `hdp-bridge serve` — the foreground daemon entrypoint (§5.5). PID-file…, Another live hdp-bridge process already holds this profile's PID file. (+13 more)
...
Cohesion: 0.20
Nodes (6): asyncio, Fault injection for the conformance suite (m1-plan.md §7). Every fault is…, Protocol-level observability for frames received by the reference node., test_invoke_frame_is_logged_before_dispatch(), _WebSocket, LogCaptureFixture
...
### Community 51 - "Hello"
Cohesion: 0.24
Nodes (12): Hello, Node → Bridge, first frame on a new connection. `hello` doubles as the node's…, parametrize, A pre-v0.3 node omits the key entirely; that must stay a valid `hello`, not a…, Optional, not untyped — same posture as a non-string `credential`., Amendments v0.3. Emitting the key as an explicit null (rather than omitting it)…, test_from_wire_tolerates_unknown_fields(), test_hello_platform_defaults_to_none_and_round_trips_present_as_null() (+4 more)

### Community 52 - "test_control.py"
Cohesion: 0.18
Nodes (21): _connect_and_hello(), ctl_conn(), _ctl_invoke(), node_client(), fixture, `hdp_bridge.control` — the plugin↔bridge Unix-socket control plane.…, Nothing-leaks invariant (hdp-spec/HDP-0.md §7): after a terminal outcome —…, M3 exposes daemon-held pending approvals over the existing control verb. (+13 more)
...
Cohesion: 0.18
Nodes (8): ControlServer, Best-effort cancel, mirroring `EmbeddedTransport.cancel` — safe to call…, Operator-only verb (`hermes hdp devices` at Task 17 was ultimately built…, Operator-only verb backing `hermes hdp audit` / `/hdp audit` — read-only, but…, Return daemon-memory approvals; none are durable while pending., Operator-only verb (FR-15, §4.4) — the CLI's `hdp-bridge devices revoke`…, Envelope, An HDP/0 frame. `payload` is opaque to this layer — Hermes…

### Community 58 - "CapabilityDescriptor"
Cohesion: 0.16
Nodes (14): _InvokeRequestLike, _PendingHandshake, Protocol, A challenge awaiting its proof (HDP-0.md Amendments v0.4). `pair_code` is set…, CapabilityDescriptor, One entry in a `capabilities` message's full-replacement list (HDP-0.md §2, §6)., _capabilities_from_wire(), CapabilitiesMsg (+6 more)

### Community 59 - "hermes_device_plugin/__init__.py"
Cohesion: 0.12
Nodes (15): _build_parser(), main(), Any, ArgumentParser, The `hermes hdp ...` operator entry point. `asyncio.run()` only appears here,…, Registers `hermes hdp {status,devices,pair,audit}` (design §2's confinement…, register_cli_command(), Any (+7 more)
...
### Community 63 - "server.py"
Cohesion: 0.18
Nodes (14): Application, ConnectionFactory, _blobs_reserved(), build_app(), _health(), _make_socket_handler(), Any, WebSocketResponse (+6 more)
...
### Community 66 - "test_lifecycle.py"
Cohesion: 0.27
Nodes (14): Node → Bridge. `ok=True` carries `data`; `ok=False` carries `error` shaped like…, ResultMsg, test_result_msg_rejects_non_bool_ok(), mint_pairing_code(), Run `hdp-bridge pair new` as a subprocess against the already-running bridge…, _echo_request(), M4 live lifecycle and version-resolution behavior over the public protocol., test_appear_then_disappear_changes_the_very_next_implicit_resolution() (+6 more)
...
Cohesion: 0.22
Nodes (9): _FakeWS, `_handle_hello`'s M2 auth branch (§3, §4.3, §4.4): M2 does not accept unpaired…, Both handshake paths run through `_register_device`, so a node that starts…, _read_audit_records(), test_hello_with_a_valid_pairing_code_pairs_and_returns_a_credential(), test_hello_with_an_invalid_pairing_code_is_auth_failed(), test_hello_with_an_unknown_credential_is_auth_failed(), test_hello_with_no_credential_is_auth_failed() (+1 more)
...
Cohesion: 0.24
Nodes (8): Connection, Returns the number of live credentials actually invalidated — `0` for an…, revoke_device(), _FakeWS, Operator-initiated revocation (FR-15, §4.4) — the four-step order enforced in…, test_revoke_fails_in_flight_invocation_with_revoked_not_device_offline(), test_revoke_invalidates_credential_sends_revoke_frame_and_fails_in_flight(), test_revoke_of_unconnected_device_still_invalidates_credential()

### Community 76 - "_serve_claimed"
Cohesion: 0.22
...
### Community 82 - "NoPairingCodeAvailableError"
Cohesion: 0.67
Nodes (3): NoPairingCodeAvailableError, RuntimeError, Every sampled code collided with a code that is still live. Only reachable with…
...
- **Why does `Envelope` connect `Envelope` to `.from_wire`, `SocketTransport`, `.new`, `._handle`, `control.py`, `test_cli.py`, `InvokeRequest`, `InvocationsMem`, `test_m4_resolution.py`, `AuditWriter`, `_ResolvedTarget`, `node.py`, `test_presence.py`, `AndroidNodeFixture`, `_NodeSession`, `asyncio`, `test_device_bound_pairing.py`, `CapabilityDescriptor`, `NodeConnection`, `_ImmediateConnection`, `test_connection_auth.py`, `_FakeWS`, `_Harness`?**
  _High betweenness centrality (0.081) - this node is a cross-community bridge._
- **Why does `Registry` connect `Registry` to `.new`, `control.py`, `test_cli.py`, `InvocationsMem`, `test_m4_resolution.py`, `connect`, `_ResolvedTarget`, `test_plugin_cli.py`, `test_server.py`, `issue_credential`, `test_presence.py`, `AndroidNodeFixture`, `mint_pairing_code`, `daemon.py`, `test_device_bound_pairing.py`, `test_control.py`, `Envelope`, `CapabilityDescriptor`, `NodeConnection`, `_ImmediateConnection`, `test_connection_auth.py`, `test_revocation.py`, `_serve_claimed`, `_FakeWS`, `_Harness`, `PolicyEngine`, `.__init__`?**
  _High betweenness centrality (0.067) - this node is a cross-community bridge._
- **Why does `NodeConnection` connect `NodeConnection` to `.from_wire`, `Registry`, `HdpServer`, `control.py`, `InvocationsMem`, `AuditWriter`, `connect`, `_ResolvedTarget`, `test_server.py`, `messages.py`, `issue_credential`, `test_presence.py`, `AndroidNodeFixture`, `mint_pairing_code`, `daemon.py`, `test_device_bound_pairing.py`, `Hello`, `test_control.py`, `Envelope`, `CapabilityDescriptor`, `server.py`, `test_lifecycle.py`, `test_connection_auth.py`, `test_revocation.py`, `_serve_claimed`, `_FakeWS`, `_Harness`, `PolicyEngine`?**
